...
本文档用于帮助开发者理解 AI Task Orchestrator 的项目结构、核心包职责、核心类职责，以及一次任务从 HTTP 创建到异步执行、重试、取消和超时处理的完整调用链路。
...
- `TaskDetailResponse`：任务详情响应体，返回任务状态、错误信息、重试字段、超时字段和时间信息。
...
9. 模拟执行任务
10. 成功：RUNNING -> SUCCESS
11. 失败可重试：RUNNING -> RETRY_PENDING
12. RetryScheduler 到期重新投递
13. 重试耗尽：RUNNING -> FAILED
14. 用户取消：进入 CANCELLED
15. TimeoutScheduler 超时：RUNNING -> FAILED
```

## 十四、当前架构边界

当前项目已经实现的是任务编排底座，不是完整 AI Agent 平台。
